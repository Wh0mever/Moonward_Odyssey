...
interface PlayerProps {
    ...
    onShoot: (targetId: string | null, isHeadshot: boolean, hitPoint: Vector3, isFirstHit?: boolean) => void;
...
export const Player: React.FC<PlayerProps> = ({
    ...
    // PointerLockControls ref for ESC unlock
    ...
    // Track if equipped for fireOnce closure
    const isEquippedRef = useRef(isEquipped);
    useEffect(() => { isEquippedRef.current = isEquipped; }, [isEquipped]);

    // Unlock pointer on death
    useEffect(() => {
        if (isDead) {
            // Force unlock pointer with both methods for reliability
            if (controlsRef.current) {
    ...
    // Fire single shot function (extracted for reuse)
    const fireOnce = () => {
        if (stats.current.ammo <= 0 || !isEquippedRef.current) return;

        setIsShooting(true);
        setTimeout(() => setIsShooting(false), 100);

        // PLAY LASER SHOT SOUND
        soundService.playLaserShot();
        ...
        // BULLET PENETRATION: Collect ALL enemy hits along the ray
        const enemyHits: { id: string; isHeadshot: boolean; point: Vector3 }[] = [];
        let lastHitPoint = new Vector3().copy(camera.position).add(dir.clone().multiplyScalar(100));
        ...
        for (let i = 0; i < intersects.length; i++) {
            ...
            if (hit.object.userData.type === 'ENEMY') {
                ...
                // CONTINUE - don't break, bullet penetrates through enemies
            } else if (hit.object.userData.type === 'TERRAIN' || hit.object.userData.type === 'CACTUS') {
                // ONLY stop on terrain or solid obstacles
                lastHitPoint = hit.point.clone();
                break;
            }
            // For any other object type, continue (bullet passes through)
        }

        // Call onShoot for EACH enemy hit (penetration)
        if (enemyHits.length > 0) {
            enemyHits.forEach((hit, index) => {
                // Only first hit deducts ammo
                onShootRef.current(hit.id, hit.isHeadshot, hit.point, index === 0);
            });
        } else {
            onShootRef.current(null, false, lastHitPoint, true);
        }

        setLaserTarget(lastHitPoint);
    ...
    useEffect(() => {
        ...
        // AUTO-FIRE: Hold LMB to continuously fire
        const handleMouseDown = () => {
            if (!isDead && isEquippedRef.current && stats.current.ammo > 0) {
                fireOnce();
                fireIntervalRef.current = window.setInterval(() => {
                    if (stats.current.ammo > 0 && isEquippedRef.current) {
                        fireOnce();
                    } else {
    ...
    }, [isDead, camera, scene, sensitivity]);
        ...
        <>
            {!isDead && (
                <>
                    ...
                    {/* Weapon renders itself in world space, positions relative to camera */}
                    <Weapon isShooting={isShooting} isEquipped={isEquipped} laserTarget={laserTarget} />

                    {/* Flashlight attached to camera via portal */}
                    {createPortal(
                        <spotLight
                            position={[0.2, -0.1, 0]}
                            intensity={3}

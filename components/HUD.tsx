...
const HUD: React.FC<HUDProps> = ({ stats, meteors, enemies, collectibles, gameState, onRestart, deathReason, isEquipped, isHovering, timeRemaining }) => {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none p-6 flex flex-col justify-between">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
        ...
        {/* PICKUP HINT - Shows when near collectible */}
        {isHovering && (
          <div className="absolute top-12 left-1/2 -translate-x-1/2 whitespace-nowrap animate-pulse">
            <div className="bg-green-600/90 backdrop-blur px-4 py-2 rounded-lg border border-green-400 shadow-lg">
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20">
        <div className={`bg-black/70 backdrop-blur-md px-6 py-2 rounded-lg border ${timeRemaining <= 60 ? 'border-red-500 animate-pulse' : 'border-cyan-500/30'}`}>
          <p className="text-[10px] text-gray-400 uppercase text-center">Mission Time</p>
          <p className={`text-3xl font-bold font-mono text-center ${timeRemaining <= 60 ? 'text-red-500' : 'text-white'}`}>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="bg-black/60 backdrop-blur-md p-3 rounded-lg border border-purple-500/30 w-64 mt-2">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className={`text-xs font-mono ${stats.artifacts >= 5 ? 'text-green-400' : 'text-blue-300'}`}>{stats.artifacts}/5</span>
            </div>
      ...
      {stats.xp === 0 && stats.level === 1 && showTutorial && (
        <div className="absolute top-24 left-1/2 -translate-x-1/2 text-center pointer-events-none animate-fade-in-down">
          <div className="bg-black/80 backdrop-blur px-6 py-4 rounded-xl border border-cyan-500/30 shadow-[0_0_20px_rgba(0,255,255,0.2)]">
            ...
            <p className="text-gray-300 text-sm mb-2">Find <span className="text-purple-400">100 Fuel</span> AND <span className="text-blue-400">5 Artifacts</span></p>
